...
包括算数类型(bool, char, int, float, double, etc.)和空类型(void)
...
- 非布尔类型的算数值 -> 布尔类型，0为false，非0为true
- 整型 -> 浮点类型，浮点数的尾数部分用于表示整型数的有效数字，因此当整形数的有效位数过多时，可能会损失精度
- 超出表示范围的数 -> 无符号类型，结果是初始值对无符号类型表示数值总数取模后的余数
- 超出表示范围的数 -> 带符号类型，结果是未定义的
- 当一个算数表达式中既含有无符号数，又有有符号整型数，且无符号数的尺寸>=有符号数的尺寸时，有符号数会被转换成无符号数

##### 字面值常量

- 整型字面值。默认情况下，十进制字面值是带符号数，八进制和十六进制字面值可能是带符号的也能是无符号的
- 浮点型字面值。默认情况下是double
...
- 默认初始化：如果定义变量时没有指定初值，则变量被默认初始化，默认值由变量类型决定，同时定义变量的位置也会对此由影响
...
复合类型时指基于其他类型定义的类型。C++由几种符合类型，本章将介绍其中的两种：引用和指针
...
2. 普通for
   ...
   ```cpp
   string s = "cukoo";
   for (delctype(s.size()) i = 0; i < s.size(); i++) {
       cout << s[i] << endl;
   }
   ```
...
- 标准库类型vector（向量）表示对象的集合，其中所有的对象类型都相同，且每个对象都有一个与之对应的索引
...
- 在很多用到数组名字的地方，编译都会自动地将其替换为一个指向数组首元素地指针

- 换言之，在大多数表达式中，使用数组类型地对象其实是使用一个指向该数组首元素地指针
...
- 只要指针指向的时数组的元素（或者是一个尾后指针），都可以执行下标运算
...
严格来说，C++没有多维数组，通常所说的多为数组其实是数组的数组
...
- 由4种运算符明确规定了运算对象的求值顺序，分别是逻辑与运算符、逻辑或运算符、条件运算符和逗号运算符
...
- sizeof运算符返回一条表达式或一个类型名字所占的字节数，满足右集合率，其所得的值是一个size_t类型的常量表达式
...
- 对于运算对象为一个无符号类型和一个有符号类型的情形，如果无符号不小于带符号，则转成无符号；如果带符号大于无符号，如果无符号类型的所有值都能存在该带符号类型中，则转成带符号，否则转成无符号
...
- 可以将main函数定义成int main(int argc, char *argv[]) {...}，这样就可以在命令行给main传递参数。第二个形参argv数一个数组，它的元素是指向C风格字符串的指针；第一个形参argc表示数组中字符串的数量。argv的第一个元素指向程序的名字或者一个空字符串，接下来的元素依次传递命令行提供的实参，最后一个指针之后的元素值保证为0
...
- 将函数指定为内联(inline)函数后，编译过程中会将它在每个调用点上“内联地”展开，因此内敛函数可以避免函数调用的开销
...
- 成员函数通过一个名为this的额外的隐式参数来访问调用它的那个对象，this是请求该函数的对象地址（常量指针，有顶层const）
...
- 如果不主动定义拷贝、赋值、析构这些操作，编译器将替我们生成它们。一般来说，编译器生成的版本将对每个成员都执行这些操作
- 自定义操作的有关知识在第13章
...
- 缓冲机制使得操作及系统可以将程序的多个输出操作组合成单一的系统级写操作。由于设备的写操作可能很耗时，这可以性能提升
...
- ate和binary模式可用于任何类型的文件流对象，且可以于其他任何文件模式组合使用
- 可以通过位与操作符对文件模式进行组合
...
| string               | 与vector相似的容器，但专门用于保存字符。随机访问快。在尾部进行插入/删除操作快 |
...
- 迭代器支持的运算只适用于string、vector、deque和array的迭代器
...
| s.replace(range, args) | 删除s中该范围range内的字符，替换为args指定的字符。range是一个下标和一个长度，或者是一对指向s的迭代器。返回一个指向s的引用 |
| args可能是下列形式之一 |                                                              |
| str                    | 字符串str                                                    |
...
| string和数值之间的转换 | 说明                                                         |
| ---------------------- | ------------------------------------------------------------ |
| to_string(val)         | 一组重载函数，返回数值val和string表示。val可以是任何算术类型。对每个浮点类型和int或更大的整型，都有相应版本的to_string。与往常一样，小整型会被提升 |
...
2. 队列适配器(queue、priority_queue)
   ...
   | ------------------------- | ------------------------------------------------------------ |
   | q.pop()                   | 返回queue的首元素或priority_queue的最高优先级元素，但不删除该元素 |
...
5. find_if(b, e, predicate)
   ...
   对输入范围内的每个元素调用谓词，返回指向第一个使谓词返回非0值的元素的迭代器，如果不存在这样的元素，则返回e。
...
插入迭代器器是一种迭代器适配器，它接受一个容器，生成一个迭代器，能实现向给定容器添加元素。当我们对一个插入迭代器进行赋值时，该迭代器调用容器操作来向给定容器的指定位置插入一个元素。
...
- 反向迭代器和普通迭代器间的关系：
  ...
  ![image-20230616110646107](https://raw.githubusercontent.com/Cukoo-Li/typora-images/main/2023/06/upgit_20230616_1686884807.png)
...
- 关联容器支持高效的关键字查找和访问
- 两个主要的关联容器类型是map和set：map的元素是键-值对，set中每个元素只包含一个键
...
- 可以将关联容器初始化为另一种同类型容器的拷贝，或是从一个值范围来初始化关联容器，只要这些值能转化为容器所需类型即可
- C++11可以对关联容器进行列表初始化
...
- 静态内存用来保存局部static对象、类static数据成员、全局对象；栈内存用来保存局部对象
...
| unique_ptr\<T> u(q)                             | 用类型为T*的指针q初始化u                                     |
| unique_ptr\<T, D> u(q, d)                       | 用类型为T*的指针q初始化u，用类型为D的对象d代替delete         |
...
- 在拷贝初始化过程中，编译器可以（但不是必须）跳过拷贝/移动构造函数，直接创建对象。即便如此，拷贝/移动给构造函数必须是存在且可访问的
