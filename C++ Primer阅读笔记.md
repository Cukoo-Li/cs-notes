...
| c.insert(b, e)<br />c.insert(il)       | b和e是迭代器，表示一个value_type类型值的范围；il是这种值的花括号列表。函数返回void<br />对于map和set，只插入关键字不再c中的元素<br />对于multimap和multiset，则会插入范围中的每个元素 |
