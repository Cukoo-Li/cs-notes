...
- 除了构造函数和赋值运算符以外，根据实际需要，一个成员函数也可以同时提供拷贝和移动版本，拷贝版本接受一个指向const的左值引用，移动版本接受一个指向非const的右值引用。
...
- 之所以是非常量，是因为向流写入内容会改变其状态。
- 之所以是引用，是因为我们无法直接拷贝一个 `ostream` 对象。

第二个形参一般来说是一个常量的引用。

- 之所以是引用，是因为我们希望避免拷贝实参。
- 之所以该形参可以是常量，是因为打印对象一般不会改变对象的内容。

##### 重载输入运算符 >>

通常情况下，输入运算符的第一个形参是一个非常量 `istream` 对象的引用，第二个形参是将要读入到的（非常量）对象的引用。
...
> 标准库定义一组表示算数运算符、关系运算符和逻辑运算符的类模板（例如 `plus<T>`、`greater<T>`、`logical_and<T>` 等），其中定义了函数调用运算符，我们可以在算法中使用标准库函数对象。

在 C++ 中有几种可调用对象：函数指针、lambda 表达式、`std::bind` 创建的对象、函数对象等。它们类型不同，但可能具有相同的调用形式，我们可以使用类模板 `std::function` 将具有指定调用形式的可调用对象包装起来。
...
>
> 当我们初始化其中的某个数据成员时，实际上就是在这段内存中进行初始化操作。对于基本数据类型来说，初始化就是简单地写值，这对于编译器来说是很容易实现的；而对于类类型来说，需要通过构造函数来完成初始化，因此要求程序员显式使用定位 `new` 表达式来调用构造函数。至于销毁某个数据成员，也是类似的道理。
