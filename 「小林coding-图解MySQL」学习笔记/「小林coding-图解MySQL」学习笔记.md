...
> undo log 和 redo log 都是 InnoDB 引擎实现的，而 binlog 是 MySQL 的服务层实现的。

binlog 是一种用于备份恢复、主从复制的日志，记录了所有对表结构或表数据的更新操作。
...
> Buffer Pool 对缓存页的管理跟操作系统中的内存管理很类似。
