...
对于联合索引（a, b），在执行 `select * from table where a > 1 and b = 2` 语句的时候，在联合索引中查找时只有 a 字段被用到，那在联合索引中找到第一个满足条件 `a > 1` 的记录后，还需要通过遍历的方式判断其他条件是否满足（在这里是 `b = 2`）。那么，是在联合索引里判断呢？还是回主键索引去判断呢？
...
![图片](images/640-17250031477932.webp)
...
- 隔离性：允许多个事务并发执行，操作相同的数据不会出现数据不一致的情况，每个事务逻辑上都有一个独立的数据空间，各个并发事务之间是隔离的。
...
- 读未提交（read uncommitted）：一个事务还没提交时，它所做的修改就能被其他事务看到。
- 读提交（read committed）：一个事务提交之后，它所做的修改才能被其他事务看到。
- 可重复读（repeatable read）：一个事务执行过程中看到的数据，一直跟这个事务开启时看到的数据是一致的（对于快照读来说）。
- 串行化（serializable）：会对记录加上读写锁，在多个事务对同一条记录进行读写操作时，如果发生了读写冲突的时候，后访问的事务必须等待。
...
当一个事务创建 Read View 后，根据记录中的 `trx_id` 字段以及 Read View 中的字段，可以知道更新该记录的事务在截至创建 Read View 之时，是“已提交的事务”，还是“未提交或未启动的事务”，从而判断这条记录对于当前事务是否可见。
...
- 临键锁（Next-Key Lock）
  ...
  临键锁由一个记录锁和一个间隙锁组成，不仅锁住了一个开区间，还锁住了一条记录，对应一个左闭右开的区间。
...
Undo Log 是一种用于撤销回退的日志，记录了每个事务在事务期间对数据所做的修改操作的逆操作。
...
Redo Log 是一种用于重做的日志，记录了每个事务在事务期间对哪些数据页做了哪些修改。
...
Binlog 是一种用于备份恢复、主从复制的日志，记录了每个事务对数据的所有修改操作。
...
- 在 prepare 阶段，将 XID（内部 XA 事务的 ID）写入到 Redo Log 中，同时将 Redo Log 对应的事务状态设置为 prepare，然后将 Redo Log 写入磁盘。
...
主从复制是一种数据复制技术，用于在多个数据库服务器之间同步数据。在主从复制架构中，一个服务器被设置为主服务器（Master），充当数据源，其他服务器被设置为从服务器（Slave），用来复制主服务器的数据。

主从复制主要是基于 Binlog 实现的，它的实现流程如下：

1. 主数据库的修改记录被记录到 Binlog 中
2. 从数据库的 IO 线程读取主数据库的 Binlog，将其内容拷贝到 Relay Log（中继日志）中
