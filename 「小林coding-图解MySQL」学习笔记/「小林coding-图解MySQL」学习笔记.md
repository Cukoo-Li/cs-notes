...
MVCC，多版本并发控制，通过维护记录的多个版本，使得并发事务之间的读写操作没有冲突，快照为 MySQL 实现 MVCC 提供了一个非阻塞读功能。MVCC 的具体实现，还需要依赖数据库表结构中的两个隐藏字段、Read View 和 undo log 版本链。

当前读：读取的是记录的最新版本，读取时还要保证其他并发事务不能修改当前记录，会对读取的记录进行加锁。

快照读：不加锁的就是快照读，读取的是记录的可见版本，有可能是历史版本数据，是非阻塞读。

> - 读已提交：每次 select，都生成一个快照读（Read View）
...
Read View 是在 MVCC 中保证事务读取到正确数据的依据。
...
当一个事务创建 Read View 后，对于某一条记录中的 `trx_id`，根据当前事务的 Read View 中的字段，可以知道更新该记录的事务是“已提交的事务”、“未提交的事务”、“未启动的事务”三者中的一种，从而判断这条记录对于当前事务是否可见。
